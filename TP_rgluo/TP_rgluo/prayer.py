...
def drawTitle(app,canvas):
    ...
    h=app.height
    txtSize=w//20
    canvas.create_text(7*w/24,h/10, text="Prayer Requests", anchor="w",
                        font=f'Helvetica {txtSize} bold', fill="white")

#draw main space with prayer requests
def drawPrayer(app,canvas):
    w=app.width
    h=app.height
...
def drawSelection(app,canvas):
    w=app.width
    h=app.height
    create_good_rectangle(canvas,-9,49*h/80,21*w/100,11*h/16,2,color="#BDE5F5")

def drawAll(app,canvas):
    drawTitle(app,canvas)
    drawSelection(app,canvas)
    drawPrayer(app,canvas)