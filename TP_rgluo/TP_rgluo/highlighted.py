...
def drawTitle(app,canvas):
    ...
    h=app.height
    txtSize=w//20
    canvas.create_text(7*w/24,h/10, text="Highlighted", anchor="w",
                        font=f'Helvetica {txtSize} bold', fill="white")

#draw main space wiht highlighted verses and notes
def drawHighlighted(app,canvas):
    w=app.width
    h=app.height
    create_good_rectangle(canvas,7*w/24,7*h/40, 29*w/30, 19*h/20, 20,
                            color="#ECF8FF" )
#draws selected tab
def drawSelection(app,canvas):
    w=app.width
    h=app.height
    create_good_rectangle(canvas,-9,41*h/80,21*w/100,47*h/80,2,color="#BDE5F5")

def drawAll(app,canvas):
    drawTitle(app,canvas)
    drawSelection(app,canvas)
    drawHighlighted(app,canvas)
